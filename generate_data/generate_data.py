...
class GraphRAGDataGenerator:
    ...
    def __init__(self):
        ...
        self.config = {
            "skills": {
                "proficiency_levels": os.getenv("SKILL_PROFICIENCY_LEVELS").split(","),
                "proficiency_weights": [
                    int(x) for x in os.getenv("SKILL_PROFICIENCY_WEIGHTS").split(",")
                ],
            },
            "project_requirements": {
                "min_requirements": int(os.getenv("PROJECT_REQ_MIN")),
                "max_requirements": int(os.getenv("PROJECT_REQ_MAX")),
                "mandatory_probability": float(os.getenv("PROJECT_REQ_MANDATORY_PROB")),
            },
            "assignment": {
                "assignment_probability": float(os.getenv("ASSIGN_PROB")),
                "assignment_end_days_before_min": int(os.getenv("ASSIGN_END_MIN")),
                "assignment_end_days_before_max": int(os.getenv("ASSIGN_END_MAX")),
            },
            "output": {
                "programmers_dir": os.getenv("OUTPUT_PROGRAMMERS"),
                "projects_dir": os.getenv("OUTPUT_PROJECTS"),
                "rfps_dir": os.getenv("OUTPUT_RFPS"),
            },
            "generation": {
                "num_programmers": int(os.getenv("GEN_NUM_PROGRAMMERS")),
                "num_projects": int(os.getenv("GEN_NUM_PROJECTS")),
                "num_rfps": int(os.getenv("GEN_NUM_RFPS")),
            },
        }
    ...
    def recreate_dir(self, path: str):
        """Usuń katalog jeśli istnieje i utwórz go od nowa."""
        if os.path.exists(path):
            shutil.rmtree(path)  # usuwa katalog + całą zawartość
    ...
    def _generate_skills(self) -> List[dict]:
        ...
        for skill in selected_skills:
            ...
            skills_with_proficiency.append({"name": skill, "proficiency": proficiency})
    ...
    def generate_projects(
        self, num_projects: int = 20, programmer_profiles: List[dict] = None
    ) -> List[dict]:
        ...
        for i in range(num_projects):
            ...
            if programmer_profiles:
                ...
                for skill in required_skills:
                    ...
                    is_mandatory = (
                        random.random()
                        < self.config["project_requirements"]["mandatory_probability"]
                    )
                    requirements.append(
                        {
                            "skill_name": skill,
                            "min_proficiency": random.choice(
                                self.config["skills"]["proficiency_levels"]
                            ),
        ...
        if programmer_profiles:
            projects = self._assign_programmers_to_projects(
                projects, programmer_profiles
            )

        return projects

    def _assign_programmers_to_projects(
        self, projects: List[dict], programmer_profiles: List[dict]
    ) -> List[dict]:
        ...
        def is_available(programmer_id, start_date, end_date):
            ...
            for assignment in assignments:
                assign_start = datetime.fromisoformat(
        ...
        for project in assignable_projects:
            ...
            for programmer in selected_programmers:
                ...
                if project["status"] == "completed":
                    ...
                    assignment_end_date = estimated_end - timedelta(
                        days=days_before_end
                    )
                    assignment_end = assignment_end_date.isoformat()
    ...
    def generate_rfps(self, num_rfps: int = 3) -> List[dict]:
        ...
        for i in range(num_rfps):
            ...
            rfp = {
                ...
                "requirements": requirements,
                "location": fake.city(),
                "remote_allowed": random.choice(
                    [True, True, False]
                ),  # More likely to allow remote
            }
            rfps.append(rfp)
...
"""
    ...
    def save_cv_as_pdf(
        self, markdown_content: str, filename: str, output_dir: str
    ) -> str:
        ...
        HTML(string=html_content).write_pdf(
            pdf_path, stylesheets=[CSS(string=css_content)]
        )
    ...
    def generate_all_data(
        self, num_programmers: int = 10, num_projects: int = 20, num_rfps: int = 3
    ) -> dict:
        ...
        self.recreate_dir(programmers_dir)
        self.recreate_dir(rfps_dir)
        self.recreate_dir(projects_dir)
        ...
        print(
            f"Generating {num_projects} project records with programmer assignments..."
        )
        ...
        for i, rfp in enumerate(rfps, 1):
            ...
            # Save as PDF
            safe_title = (
                rfp["title"].replace(" ", "_").replace(".", "").replace("/", "_")
            )
...
def main():
    ...
    try:
        ...
        result = generator.generate_all_data(
            config["num_programmers"], config["num_projects"], config["num_rfps"]
        )
