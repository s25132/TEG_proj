...
class SystemComparator:
    ...
    def __init__(self):
        ...
        logger.info(f"✓ System Comparator initialized (backend={self.backend_url})")
    ...
    async def run_full_comparison(self) -> Dict[str, Any]:
        ...
        for i, op_data in enumerate(operational_data):
            ...
            # Wyciągamy wyniki dla konkretnego pytania
    ...
    def display_results(self, comparison_data: Dict[str, Any]) -> None:
        ...
        print(f"\nWIN RATES:")
        print(f"   • GraphRAG Win Rate: {perf['graphrag_win_rate']:.1%}")
        print(f"   • Naive RAG Win Rate: {perf['naive_rag_win_rate']:.1%}")
        print(f"   • Ties: {perf['ties']}")

        print(f"\nAVERAGE QUALITY SCORES:")
        print(f"   • GraphRAG:  {scores['graphrag_avg']:.4f}")
        print(f"   • Naive RAG: {scores['naive_rag_avg']:.4f}")
        ...
        for category, stats in summary["category_breakdown"].items():
            ...
            if total > 0:
                ...
                print(f"   • {category.title():<15}: GraphRAG {stats['graph_wins']} wins ({g_rate:.0%}) | Naive {stats['naive_wins']} wins ({n_rate:.0%})")
...
async def wait_for_healthcheck(
    ...
    """
    Sprawdza endpoint /healthcheck aż zwróci status == 'ok'
    lub wyczerpie liczbę prób.

    :param url: np. "http://localhost:8000/healthcheck"
    :param retries: liczba prób
    :param delay: czas oczekiwania między próbami (sekundy)
    :param timeout: timeout HTTP
    :return: True jeśli healthcheck OK, inaczej False
    """
...
async def main():
    ...
    if not is_up:
        raise RuntimeError("API nie wystartowało")
    ...
    try:
        ...
        print("\n💥 Workflow failed: see logs (traceback printed to logger)")
